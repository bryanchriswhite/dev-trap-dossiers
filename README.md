...
**Currently tracking:** an active developer-targeting operation matching the publicly-documented **"Contagious Interview" TTP cluster** (fake-recruiter → clone/run repo or open supplied archive → stealer-loader). **≥43 confirmed repository instances** (16 AjunaVerse-family + 27 realfraction-family) across **12 operator-owned GitHub organizations** (3 AjunaVerse-family + 9 realfraction-family) plus several operator-owned and likely-candidate-fooled individual accounts. Loader code spans **at least three distinct GitHub-hosted generations** (two AjunaVerse-family + one realfraction-family with eight internal sub-shapes A–H), and the realfraction-family is now known to share Vercel hosting and an RCE primitive with the AjunaVerse-family (a cross-generation operator overlap; see incident 2026-05-18-realfraction §7.5 / §7.10). A newer DAAM case documents a LimeWire-delivered ZIP with bundled Git hook execution, Bitbucket installer instructions, and a hidden `.svn/.DS_Store` JavaScript payload, with strong technical overlap with Contagious Interview / OtterCookie infrastructure. C2/dropper infrastructure identified across cases includes Vercel-hosted deployments, `ipregionchecker.com`, `isillegalregion.com`, `cookie-xi-seven.vercel.app`, `ip-check-api.vercel.app`, `iploglab.store`, and `216.126.225.243:8085/8086/8087`.
...
If you were sent a ZIP/archive rather than a GitHub URL — especially via a file-sharing service — and it contains a complete `.git` directory, branch-switching instructions, Bitbucket installer commands, or hidden/system-looking JavaScript such as `.svn/.DS_Store`, read the **[DAAM briefing](./incidents/2026-08-12-daam-ai-agent-marketplace/briefing-for-developers.md)** before touching it.
...
| [slobodanmargetic988](https://github.com/slobodanmargetic988) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 25 repos since 2018; mixed real-developer history. Hosts sub-shape B realfraction-family loader at `server/controllers/paymentController.js`. |
| [LandinLin](https://github.com/LandinLin) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 9 repos since 2018. Hosts sub-shape C loader. |
| [devcode8](https://github.com/devcode8) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 62 repos since 2022. Hosts sub-shape C loader. |
| [eastmade](https://github.com/eastmade) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 20 repos since 2017. Hosts sub-shape F loader. |
| [MBhatti26](https://github.com/MBhatti26) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 10 repos since 2025-03. Hosts sub-shape F loader on `Purrtal`. |
| [fabiolin](https://github.com/fabiolin) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 10 repos since 2014. Hosts sub-shape G template on `schoolmgmt`. |
| [sharmapranay38](https://github.com/sharmapranay38) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 69 repos since 2020. Hosts sub-shape G loader. |
| [shri33](https://github.com/shri33) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 72 repos since 2020. Hosts sub-shape G/H hybrid loader. |
| [Paulooo0](https://github.com/Paulooo0) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 41 repos since 2022. Hosts sub-shape G template. |
| [KagiyamaWeb](https://github.com/KagiyamaWeb) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 14 repos since 2020. Hosts sub-shape G template. |
| [Wilovy09](https://github.com/Wilovy09) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 70 repos since 2020. Hosts sub-shape G template. |
| [pablodiaz2799](https://github.com/pablodiaz2799) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 17 repos since 2018. Hosts sub-shape G template. |
| [Jay-Sojitra](https://github.com/Jay-Sojitra) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 75 repos since 2021. Hosts sub-shape H loader (Function.constructor RCE). |
| [ahmedraza90](https://github.com/ahmedraza90) | user | **Likely candidate-fooled** (investigate, don't suspend; mechanism: the operator likely sent these accounts' owners a take-home assignment that embedded the loader, and the candidate pushed their working copy) | — | 36 repos since 2020. Hosts sub-shape H loader. |
...
| 2026-08-12 | [daam-ai-agent-marketplace](./incidents/2026-08-12-daam-ai-agent-marketplace/) | confirmed malicious; LimeWire-delivered DAAM AI-agent marketplace ZIP with three execution paths: bundled Git post-checkout hook to `iploglab.store`, README Bitbucket installer instructions, and backend startup importing hidden obfuscated JS at `.svn/.DS_Store`; strong Contagious Interview / OtterCookie infrastructure overlap | [case file](./incidents/2026-08-12-daam-ai-agent-marketplace/README.md) · [for devs](./incidents/2026-08-12-daam-ai-agent-marketplace/briefing-for-developers.md) · [LimeWire abuse](./incidents/2026-08-12-daam-ai-agent-marketplace/abuse-report-limewire.md) · [LinkedIn abuse](./incidents/2026-08-12-daam-ai-agent-marketplace/abuse-report-linkedin.md) · [Calendly abuse](./incidents/2026-08-12-daam-ai-agent-marketplace/abuse-report-calendly.md) · [Bitbucket abuse](./incidents/2026-08-12-daam-ai-agent-marketplace/abuse-report-bitbucket.md) · [authorities report](./incidents/2026-08-12-daam-ai-agent-marketplace/abuse-report-authorities.md) · [IOCs](./incidents/2026-08-12-daam-ai-agent-marketplace/iocs.csv) · [rules](./incidents/2026-08-12-daam-ai-agent-marketplace/detection-rules.md) |
...
- If a derivative type doesn't apply to a given incident (e.g., no Vercel-hosted C2 → no Vercel abuse report; no operator-identity Gmail observed → no Gmail abuse report; no domain whose abuse path is registrar-level revocation → no registrar abuse report), omit the file rather than leaving an empty placeholder. Conversely, if a new platform-level abuse path becomes relevant for an incident, add a new `abuse-report-<platform>.md` file using the existing reports' template structure.
